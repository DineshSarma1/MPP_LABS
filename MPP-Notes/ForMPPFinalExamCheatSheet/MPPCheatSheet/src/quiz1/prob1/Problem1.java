...
public class Problem1 {
	...
	public static List<String> elementsInJustOne(List<String> list1, List<String> list2) {
		...
		return Stream.concat(list1.stream(), list2.stream())
				.filter(str -> !(list2.contains(str) && list1.contains(str))).collect(Collectors.toList());

	}

	public static List<String> unionTwoLists(List<String> list1, List<String> list2) {
		return Stream.concat(list1.stream(), list2.stream()).distinct().collect(Collectors.toList());
	}

	public static List<String> intersectionTwoList(List<String> list1, List<String> list2) {
		return Stream.concat(list1.stream(), list2.stream())
				.filter(name -> (list2.contains(name) && list1.contains(name))).distinct().collect(Collectors.toList());
	}
	...
	public static List<String> getAllFairfieldCustomers(List<Customer> list) {
		// implement
		return list.stream().filter(customer -> customer.getCity().equals("Fairfield"))
				.sorted(Comparator.comparing(Customer::getName)).map(customer -> customer.getName())
	...
	private static void testFirstMethod() {
	    ...
	    System.out.println("After Union of two list of String: " + unionTwoLists(list1, list2));
	    System.out.println("After Intersection of two list of String: " + intersectionTwoList(list1, list2));
	  }
